...
func Default(db *sql.DB, tableName string, opts ...Option) *Hook {
	extraItems := make([]*ExecExtraItem, 4)
	extraItems = WithLevelExecExtra(extraItems)
	extraItems = WithLogtimeExecExtra(extraItems)
	extraItems = WithMessageExecExtra(extraItems)
	extraItems = WithDataExecExtra(extraItems)

	return NewWithExtra(db, tableName, extraItems, opts...)
}

// DefaultWithExtra create a default mysql hook with extra items
func NewWithExtra(db *sql.DB, tableName string, extraItems []*ExecExtraItem, opts ...Option) *Hook {
	var options []Option
